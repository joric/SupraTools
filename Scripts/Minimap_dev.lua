...
local function loadImages(bgContainer)
    ...
    if material and material:IsValid() then
        ...
        print("### LOADED HDR MATERIAL", dmi:GetFullName())
    end
...
local function createMinimap()
    ...
    if mapWidget and mapWidget:IsValid() then return end
    ...
    widget.WidgetTree.RootWidget = StaticConstructObject(StaticFindObject("/Script/UMG.CanvasPanel"), widget.WidgetTree, FName("RootCanvas"))
    local rootCanvas = widget.WidgetTree.RootWidget

    -- RetainerBox: visible minimap area
    local retainer = StaticConstructObject(StaticFindObject("/Script/UMG.RetainerBox"), rootCanvas, FName("DotRetainer"))
    local retainerSlot = rootCanvas:AddChildToCanvas(retainer)
    retainerSlot:SetSize(widgetSize)
    retainerSlot:SetAnchors({Minimum={X=0.5,Y=0.5}, Maximum={X=0.5,Y=0.5}})
    retainerSlot:SetAlignment({X=0.5,Y=0.5})

    local mpath = "/Game/Mods/SupraMods/M_RadialFrame.M_RadialFrame"
    LoadAsset(mpath)
    local material = StaticFindObject(mpath)
    retainer:SetEffectMaterial(material)

    --[[
    local material = StaticFindObject("/Engine/EngineMaterials/Widget3DPassThrough.Widget3DPassThrough")
    local matlib = StaticFindObject("/Script/Engine.Default__KismetMaterialLibrary")
    local world = UEHelpers.GetWorld()
    local dmi = matlib:CreateDynamicMaterialInstance(world, material, FName("mapMaterial"), 0)
    local texture = StaticFindObject("/Game/Blueprints/PlayerMap/Textures/SupraFace.SupraFace")
    if not texture:IsValid() then
        texture = StaticFindObject("/SupraAssets/Materials/Icons/Quests/BlueRune.BlueRune")
    end
    dmi:SetTextureParameterValue(FName("Mask"), texture)
    print("### texture", texture:GetFullName(), "dmi", dmi:GetFullName())
    retainer:SetEffectMaterial(dmi)
    retainer:SetTextureParameter(FName("Texture"))
    retainer.RenderOnInvalidation = true
    retainer.bRetainRender = true
    ]]

    -- ClipCanvas inside RetainerBox
    local clipCanvas = StaticConstructObject(StaticFindObject("/Script/UMG.CanvasPanel"), retainer, FName("ClipCanvas"))

    -- DotLayer inside ClipCanvas
    local dotLayer = StaticConstructObject(StaticFindObject("/Script/UMG.CanvasPanel"), clipCanvas, FName("DotLayer"))
    local dotSlot = clipCanvas:AddChildToCanvas(dotLayer)
    dotSlot:SetSize(mapSize)
    dotSlot:SetAnchors({Minimum={X=0.5,Y=0.5}, Maximum={X=0.5,Y=0.5}})
    dotSlot:SetAlignment({X=0.5,Y=0.5})

    dotLayer:SetRenderTransformPivot({X=0.5,Y=0.5})
    dotLayer:SetRenderTranslation({X=0,Y=0})
    dotLayer:SetRenderScale({X=1,Y=1})

    retainer:SetContent(clipCanvas)

    -- Load map images and points
    loadImages(dotLayer)
    updateTiles()
    updateCachedPoints()
...
local function updateMinimap(hook, name, param)
    ...
    if retainerBox and retainerBox:IsValid() then
...
LoopAsync(5*60*1000, function() -- let's see if hooks work
    if not mapWidget or not mapWidget:IsValid() or mapWidget:GetVisibility() == HIDDEN then return end
    updateCachedPoints()
    updateMinimap()
end)
...
RegisterKeyBind(Key.R, { ModifierKey.ALT }, CreateMinimapWidget)
